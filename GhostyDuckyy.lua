function obfuscate(source,VarName,WaterMark)
    ...
    local Variable = VarName or "Sex_"
    local WM
    if source == nil then
    ...
    if typeof(WaterMark) == "string" and WaterMark ~= nil then
        WM = "    "..tostring(WaterMark).." | Secured by nfpw"
    else
        WM = "    ".."WaterMark".." | Secured by nfpw"
    end
    WM = "--[[".."\n".. tostring(WM) .."\n".."]]--".."\n\n"
    ...
    local Random_Variable = {
        TableByte = random_(math.random(2,132)),
        Table_concat = random_(math.random(2,132)),
        Loadstring = random_(math.random(2,132)),
    }
    local troll_func = [[function() ]]..add_binary(math.random(30,90), "")..[[ end]]
    local troll_var = [[local ]]..Variable..tostring(random_(math.random(15,221)))..[[ = ]]..troll_func
    local SourceByte = ""
